...
def build_state_url_dict():
    ...
    states = soup.find_all('ul', class_='dropdown-menu SearchBar-keywordSearch')

    for li in states[0].find_all('li'):
...
def get_site_instance(site_url):
    ...
    site_instance = NationalSite(category=category, name=name, address= address1+ ', ' + address2, zipcode=zipcode, phone=phone)  
...
def get_sites_for_state(state_url):
    ...
    soup = BeautifulSoup(response, 'html.parser')
    site_list = soup.find(id='list_parks').find_all('li',recursive=False)
...
def print_site(alist):
    ...
    for sites in alist:
        cnt += 1
        info = f"[{cnt}] {sites.name} ({sites.category}): {sites.address}, {sites.zipcode} "
...
def open_cache():
    ''' Opens the cache file if it exists.
...
def key_construct(baseurl, params):
    ''' Construct a key that ensures that an API request is uniquely and repeatedly identified through its baseURL and params
    ...
    param_strings.sort()
    key = baseurl + connector +  connector.join(param_strings)
    return key
...
def make_request_with_cache(baseurl, param={}):
    ...
    key_to_find = key_construct(baseurl, param)
